...
namespace WildPerception {	
    ...
    public class PeopleManager : SingletonForMonobehaviour<PeopleManager>
    {
        ...
        void SpawnHuman(Vector3 SpawnPosition)
        {
            ...
            {
                UtilExtension.QuitWithLogError($"[{nameof(PeopleManager)}] Invalid models folder : {model_PATH}");
            }
            else
            {
                ...
                {
                    UtilExtension.QuitWithLogError($"[{nameof(PeopleManager)}] File path does not contain 'Resources' folder. Check the field of Human Models_Folder in inspector");
                }
